...
| CPU/JIT | Dynarmic and memory helpers are built for current desktop/Android hosts | **Started:** a bounded ARM/Thumb runner now handles Thumb entry, BLX interworking, import trampolines, stack save, and word load/store for a generated fixture; next add Thumb-2 and enough runtime/HLE coverage for a VitaSDK sample | Physical-device Milestone 9 diagnostic pending; ordinary VitaSDK homebrew still unsupported |
...
10. Load a tiny real VitaSDK ELF and add each required Thumb-2 instruction plus libc/kernel HLE call from its bounded diagnostic trace.
